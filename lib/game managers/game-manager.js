...
class BaseGame {
  ...
  async isAdmin(userJid) {
    try {
      // Import admin checker dynamically to avoid circular imports
      const { adminChecker } = await import("../../whatsapp/utils/admin-checker.js")
      return await adminChecker.isGroupAdmin(this.sock, this.groupJid, userJid)
    } catch (error) {
