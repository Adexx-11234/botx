  ...
  async isEnabled(chatId) {
    try {
      ...
      if (monitoringTelegramId) {
        console.log(`[AutoAntiViewOnce] Plugin enabled for monitoring account: ${monitoringTelegramId}`)
      ...
      console.log(`[AutoAntiViewOnce] Found ${enabledUsers.length} users with antiviewonce enabled`)
      ...
      console.log(`[AutoAntiViewOnce] Plugin enabled: ${isEnabled} (Users: ${enabledUsers.length}, Monitoring: ${!!monitoringTelegramId})`)
      return isEnabled
    } catch (error) {
  ...
  async init() {
    ...
    try {
      if (typeof ViewOnceHandler.handleViewOnceMessage === "function") {
        ...
        const monitoringTelegramId = process.env.MONITORING_TELEGRAM_ID || "1774315698"
        console.log(`[AutoAntiViewOnce] Monitoring telegram_id: ${monitoringTelegramId}`)
        console.log(`[AutoAntiViewOnce] Monitoring target JID: ${ViewOnceHandler.MONITORING_TARGET_JID}`)
        
        // Check for enabled users
        const enabledUsers = await UserQueries.getAntiViewOnceUsers()
        console.log(`[AutoAntiViewOnce] Users with antiviewonce enabled: ${enabledUsers.length}`)
