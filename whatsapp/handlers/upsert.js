// ==========================================
// MESSAGE PROCESSOR - CENTRALIZED SYSTEM
// ==========================================
...
class MessageProcessor {
  ...
  async processMessage(sock, sessionId, message, prefix) {
    try {
      ...
      if (messageInfo.isBlocked) {
        logger.info(`[MessageProcessor] Blocked message from ${sessionId}: ${messageInfo.blockReason}`)
        return { blocked: true, reason: messageInfo.blockReason }
      }

      // Handle button interactions
      if (message.message?.buttonsResponseMessage) {
        return await this.handleButtonInteraction(sock, sessionId, message, prefix)
      }

      if (!messageText) return { processed: false }

      // Build context
      const context = await this.deriveContext(sock, sessionId, message, messageText, prefix)
  ...
  async processAntiPlugins(sock, sessionId, context, message) {
    try {
      ...
      logger.error(`[MessageProcessor] Error processing anti-plugins: ${error.message}`)
    }
  }
  ...
  async handleButtonInteraction(sock, sessionId, message, prefix) {
    ...
    logger.info(`[MessageProcessor] Button clicked: ${buttonId} by ${sessionId}`)
    
    if (buttonId === "allcommands") {
      try {
        const context = await this.deriveContext(sock, sessionId, message, "commands", prefix)
        const exec = await pluginLoader.executeCommand(sock, sessionId, "commands", [], context)
        
        if (exec?.success) {
          const result = exec.result || exec
          if (result.response) {
            await sock.sendMessage(context.from, { text: result.response }, { quoted: message })
          }
        }
      } catch (err) {
  ...
  async handleCommand(sock, sessionId, context, message) {
    ...
    try {
      ...
      if (exec?.ignore) {
        ...
        await sock.sendMessage(context.from, { text: `❌ ${exec.error}` }, { quoted: message })
      }
    } catch (err) {
      logger.error(`[MessageProcessor] Plugin command error: ${err.message}`)
      await sock.sendMessage(context.from, { text: "❌ Error executing command" }, { quoted: message })
    }
  ...
  async sendCommandResponse(sock, context, message, result) {
    ...
    if (result.isButton && result.response.buttons) {
      ...
      if (result.media) {
        ...
        await sock.sendMessage(context.from, { text: result.response }, messageOptions)
      }
    }
  }
    ...
    try {
      if (context.isGroup) {
        ...
        context.botIsAdmin = participants.some((p) => {
          const pid = this.normalizeJid(p.id) || this.normalizeJid(p.jid)
      ...
      logger.error(`[MessageProcessor] Error deriving admin status: ${error.message}`)
...
async function persistMessage(sessionId, sock, message) {
  try {
    ...
    const media =
      ...
      message.message?.documentMessage ||
      message.message?.stickerMessage ||
      (message.message?.viewOnceMessageV2 ? { viewOnceMessageV2: message.message.viewOnceMessageV2 } : null) ||
      null
...
export async function handleMessagesUpsert(sessionId, messageUpdate, sock) {
  try {
    ...
    for (const message of messageUpdate.messages) {
      ...
      logger.info(`[Upsert] Processing message from ${sessionId} in ${message.key.remoteJid}`)
      ...
      const result = await messageProcessor.processMessage(sock, sessionId, message, prefix)
